...
terraform {
  source = "${local.env.locals.kubernetes.tools.vault.source}//.?version=${local.env.locals.kubernetes.tools.vault.version}"
}
...
inputs = {
  ...
  app_name          = local.env.locals.kubernetes.tools.vault.inputs.app_name
  consul_app_name   = local.env.locals.kubernetes.tools.consul.inputs.app_name
